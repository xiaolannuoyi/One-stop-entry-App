  ...
  <div>
    <group title="离职证明上传">
        <div class="uploadBox" @click="uploadimg">
            <img width="100%" :src="leaveWork" id="one">
        </div>
...
export default {
    ...
    computed: {
        leaveWork(){
            console.log("leaveWork",this.baseURL + this.image.Quit)
            return this.baseURL + this.image.Quit;
        },
        ...mapState(['image']),
        ...mapGetters(['baseURL'])
    },
    methods:{
        ...
        comfirm(){
            let url1 = this.baseURL + 'upload/uploadQuit'
            let one = document.getElementById('one').src;
            if(one !== this.leaveWork){
